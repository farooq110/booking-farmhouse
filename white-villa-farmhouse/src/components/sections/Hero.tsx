...
export function Hero() {
  return (
    ...
    >
      {/* White Villa FarmHouse hero image SLIDER (full-bleed background).
          4 distinct images cycle with a RANDOM transition effect each
          step — fade, slide, zoom-pan (Ken Burns), or reveal (clip-path
          wipe). Honors prefers-reduced-motion by falling back to a
          single static image with a gentle cross-fade. */}
      <div className="absolute inset-0 -z-20">
        <HeroBackgroundSlider slides={[...MEDIA.hero.slides]} intervalMs={6000} />
      </div>

      {/* Subtle dark gradient so cream text stays legible over the photos */}
      <div
        className="absolute inset-0 -z-10"
        style={{
          background:
            "linear-gradient(180deg, oklch(0.22 0.018 145 / 0.55) 0%, oklch(0.22 0.018 145 / 0.35) 50%, oklch(0.22 0.018 145 / 0.78) 100%)",
        }}
      />
      ...
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col items-center px-6 py-24 text-center">
        <SectionLabel className="text-cream/90">
          <span className="h-px w-6 bg-sage-soft" />
          {FARMHOUSE.name} · Eco-Farm Retreat · Karachi
        </SectionLabel>
        ...
        <p className="mt-7 max-w-xl text-base sm:text-lg text-cream/85 text-pretty leading-relaxed">
          A private eco-farm retreat in Taiser Town, Karachi. Pool,
          gardens, gaming, and sports facilities — for your family,
          friends, and quiet weekends. Book directly with the owner.
        </p>

        <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row">
          <a
            href="#booking-form"
            className="inline-flex items-center justify-center gap-2 rounded-full bg-sage px-8 py-3.5 font-sans text-xs uppercase tracking-luxe text-on-primary transition-all hover:bg-moss elevation-1 hover:elevation-2 state-layer"
          >
        ...
        <div className="mt-16 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-cream/75">
          <TrustItem icon="eco" label="Eco-hosted by one family" />
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 hidden sm:block">
        ...
        >
          <div className="h-2 w-1 rounded-full bg-sage-soft animate-bounce" />
        </a>
      </div>
    </section>
  );
...
function TrustItem({ icon, label }: { icon: string; label: string }) {
  return (
    <span className="flex items-center gap-2">
      <span className="material-symbols-outlined text-base text-sage-soft">{icon}</span>
