...
- A registered domain name (e.g. `nazimabadindoorsports.com`) with DNS access
...
2. Configure:
   ...
   | Setting | Value |
   |---------|-------|
   | Name | `nazimabad-indoor-sports-prod` |
...
```bash
dig +short nazimabadindoorsports.com
...
ssh -i ~/path/to/your-key.pem ubuntu@nazimabadindoorsports.com
```
...
# Clone the repo
cd ~
git clone https://github.com/<you>/nazimabad-indoor-sports.git
cd nazimabad-indoor-sports/frontend

# Install deps & build
bun install
bun run build
# Output lands in ~/nazimabad-indoor-sports/frontend/dist/
```

## Step 6 — Deploy the build to /var/www

```bash
sudo mkdir -p /var/www/nazimabad-indoor-sports
sudo cp -r ~/nazimabad-indoor-sports/frontend/dist/* /var/www/nazimabad-indoor-sports/
sudo chown -R www-data:www-data /var/www/nazimabad-indoor-sports
```

## Step 7 — Configure nginx

Create `/etc/nginx/sites-available/nazimabad-indoor-sports`:

```bash
sudo tee /etc/nginx/sites-available/nazimabad-indoor-sports > /dev/null <<'EOF'
server {
    listen 80;
    listen [::]:80;
    server_name nazimabadindoorsports.com www.nazimabadindoorsports.com;

    root /var/www/nazimabad-indoor-sports;
    index index.html;
...
```bash
sudo ln -sf /etc/nginx/sites-available/nazimabad-indoor-sports /etc/nginx/sites-enabled/
sudo rm -f /etc/nginx/sites-enabled/default
sudo nginx -t        # syntax check
sudo systemctl reload nginx
```

Visit <http://nazimabadindoorsports.com> — you should see the site over HTTP.

## Step 8 — Add HTTPS with Let's Encrypt

```bash
sudo certbot --nginx -d nazimabadindoorsports.com -d www.nazimabadindoorsports.com
```
...
Visit <https://nazimabadindoorsports.com> — you should see the site over HTTPS with a
valid certificate.
...
```bash
sudo tee /usr/local/bin/deploy-nazimabad-indoor-sports > /dev/null <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
cd /home/ubuntu/nazimabad-indoor-sports
git pull
cd frontend
bun install --frozen-lockfile
bun run build
sudo rsync -a --delete dist/ /var/www/nazimabad-indoor-sports/
sudo systemctl reload nginx
echo "Deploy complete: $(date)"
EOF
sudo chmod +x /usr/local/bin/deploy-nazimabad-indoor-sports
```
...
```bash
ssh ubuntu@nazimabadindoorsports.com
deploy-nazimabad-indoor-sports
```
...
| 502 Bad Gateway | nginx is up but `/var/www/nazimabad-indoor-sports/index.html` is missing — re-run the build |
| Fonts not loading | Check `~/nazimabad-indoor-sports/frontend/index.html` has the Google Fonts `<link>` |
