# Deploy White Villa FarmHouse to AWS EC2 (without nginx)
...
bun install
bun run build
# Output: ~/nazimabad-indoor-sports/frontend/dist/
```
...
```bash
sudo tee /etc/systemd/system/nazimabad-indoor-sports.service > /dev/null <<'EOF'
[Unit]
Description=White Villa FarmHouse static file server
After=network.target

[Service]
Type=simple
User=ubuntu
WorkingDirectory=/home/ubuntu/nazimabad-indoor-sports/frontend/dist
...
sudo systemctl enable --now nazimabad-indoor-sports
sudo systemctl status nazimabad-indoor-sports
```
...
nazimabadindoorsports.com, www.nazimabadindoorsports.com {
    encode zstd gzip
...
Visit <https://nazimabadindoorsports.com> — the site should load with a valid cert
...
# Static server logs
sudo journalctl -u nazimabad-indoor-sports -f

# External check
curl -I https://nazimabadindoorsports.com
# HTTP/2 200
# server: Caddy
# strict-transport-security: max-age=31536000; includeSubDomains
```
...
bun run build
sudo rsync -a --delete dist/ /home/ubuntu/nazimabad-indoor-sports/frontend/dist/
sudo systemctl restart nazimabad-indoor-sports
echo "Deploy complete: $(date)"
EOF
sudo chmod +x /usr/local/bin/deploy-nazimabad-indoor-sports
```

Run `deploy-nazimabad-indoor-sports` whenever you want to ship a new version, or wire
...
| 502 from Caddy | Static server is down — `sudo systemctl status nazimabad-indoor-sports` |
| Site loads but no styles | Build didn't include the CSS — re-run `bun run build` |
| Browser shows old version | Cached assets — `sudo systemctl restart nazimabad-indoor-sports`, hard-refresh browser |
| `sirv: command not found` | Re-install with `bun add -g sirv-cli` |
| Port 3000 not listening | Check `sudo journalctl -u nazimabad-indoor-sports -n 50` for crash logs |
