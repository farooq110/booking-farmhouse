 ...
 *
 * FLOW:
 *   1. Component mounts → hook spawns the worker.
 *   2. Hook posts { type: "build" } to the worker.
 *   3. Worker runs buildWindmillData() (pure math, no Three.js).
 *   4. Worker posts back SerializedWindmill + transfers typed-array buffers
 *      (zero copy via the transferList).
 *   5. Hook stores the data in state.
 *   6. The Windmill component receives the data, calls buildWindmillFromData()
 *      which creates THREE.BufferGeometry / Material / Mesh without running
 *      any math.
 *
 * FALLBACK: If worker is unavailable (SSR, old browser), falls back to
 * synchronous buildWindmill() on the main thread.
 */
