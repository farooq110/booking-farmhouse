 ...
 *
 * All Float32Array / Uint32Array buffers are TRANSFERRED to the main thread
 * via postMessage(message, [transferList]) — zero copy.
 */
...
// ─── Stage 1 — build serialized windmill data ──────────────────────────
...
const MATERIALS = {
  ...
  window: {
    colorHex: "#3a5840", roughness: 0.4, metalness: 0.3,
    emissiveHex: "#8aa880", emissiveIntensity: 0.3,
  },
  hub: { colorHex: "#3a2a18", roughness: 0.6, metalness: 0.2 },
  spar: { colorHex: "#6b5536", roughness: 0.85, metalness: 0.05 },
  sail: {
    colorHex: "#f0e4c8", roughness: 0.7, metalness: 0.0, side: "double" as const,
    emissiveHex: "#a08858", emissiveIntensity: 0.05,
  },
...
export function buildWindmillData(): SerializedWindmill {
  ...
  meshes.push({ ...base, uvs: null, material: MATERIALS.stone,
    position: [0, 0.4, 0], rotation: [0, 0, 0], scale: [1, 1, 1], parentIndex: -1 });

  const mid = taperedCylinderData(0.7, 0.85, 1.0, 24);
  meshes.push({ ...mid, uvs: null, material: MATERIALS.stone,
    position: [0, 1.3, 0], rotation: [0, 0, 0], scale: [1, 1, 1], parentIndex: -1 });

  const top = taperedCylinderData(0.6, 0.7, 0.9, 24);
  meshes.push({ ...top, uvs: null, material: MATERIALS.stone,
    position: [0, 2.25, 0], rotation: [0, 0, 0], scale: [1, 1, 1], parentIndex: -1 });

  const cap = coneData(0.7, 0.7 * 0.8, 16);
  meshes.push({ ...cap, uvs: null, material: MATERIALS.cap,
    position: [0, 3.05, 0], rotation: [0, 0, 0], scale: [1, 1, 1], parentIndex: -1 });

  const door = boxData(0.3, 0.55, 0.05);
  meshes.push({ ...door, uvs: null, material: MATERIALS.door,
    position: [0, 0.3, 1.0], rotation: [0, 0, 0], scale: [1, 1, 1], parentIndex: -1 });

  const win = circleData(0.12, 16);
  const w1Angle = Math.PI * 0.25;
  meshes.push({ ...win, uvs: null, material: MATERIALS.window,
    position: [Math.cos(w1Angle) * 0.83, 1.3, Math.sin(w1Angle) * 0.83],
    rotation: [0, Math.PI + w1Angle, 0], scale: [1, 1, 1], parentIndex: -1 });
  const w2Angle = Math.PI * 0.75;
  meshes.push({ ...win, uvs: null, material: MATERIALS.window,
    position: [Math.cos(w2Angle) * 0.83, 1.3, Math.sin(w2Angle) * 0.83],
    rotation: [0, Math.PI + w2Angle, 0], scale: [1, 1, 1], parentIndex: -1 });

  const hub = sphereData(0.18, 16, 16);
  meshes.push({ ...hub, uvs: null, material: MATERIALS.hub,
    position: [0, 2.55, 0.7], rotation: [0, 0, 0], scale: [1, 1, 1], parentIndex: -1 });
  ...
  meshes.push({ ...degenerate, uvs: null,
    material: { colorHex: "#000000", roughness: 1, metalness: 0 },
    position: [0, 2.55, 0.78], rotation: [0, 0, 0], scale: [1, 1, 1],
    parentIndex: -1, isBlades: true });
  ...
  for (let i = 0; i < 4; i++) {
    ...
    meshes.push({ ...spar, uvs: null, material: MATERIALS.spar,
      position: [0, bladeLength / 2, 0], rotation: [0, 0, bladeRotation],
      scale: [1, 1, 1], parentIndex: bladesIndex });
    const sail = sailShapeData(bladeLength, bladeWidth);
    meshes.push({ ...sail, uvs: null, material: MATERIALS.sail,
      position: [0.04, 0, 0.02], rotation: [0, 0, bladeRotation],
      scale: [1, 1, 1], parentIndex: bladesIndex });
    for (let j = 1; j <= 5; j++) {
      const t = (j / 6) * bladeLength;
      const wAtT = bladeWidth * (1 - (t / bladeLength) * 0.15);
      const cross = boxData(wAtT, 0.05, 0.04);
      meshes.push({ ...cross, uvs: null, material: MATERIALS.cross,
        position: [wAtT / 2, t, 0], rotation: [0, 0, bladeRotation],
        scale: [1, 1, 1], parentIndex: bladesIndex });
    }
  }
...
// ─── Stage 2 — reconstruct THREE.Group from serialized data ───────────
...
export function buildWindmillFromData(data: SerializedWindmill): WindmillBuildResult {
  ...
  function getMaterial(spec: SerializedMaterial): THREE.Material {
    ...
    const mat = new THREE.MeshStandardMaterial({
      ...
      side: spec.side === "double" ? THREE.DoubleSide
        : spec.side === "back" ? THREE.BackSide
        : THREE.FrontSide,
