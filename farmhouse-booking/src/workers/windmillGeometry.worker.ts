 ...
 *
 * Refs: https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API
 */
...
export interface WindmillBuildRequest {
  type: "build";
}
...
self.onmessage = (e: MessageEvent<WindmillBuildRequest>) => {
  ...
  try {
    ...
    // Build the transfer list — every typed array's underlying ArrayBuffer
    // must be in this list so postMessage transfers ownership (zero copy)
    // instead of cloning.
    ...
    const res: WindmillBuiltMessage = {
      type: "built", ok: true, data, durationMs: t1 - t0,
    };

    // eslint-disable-next-line no-console
    console.debug(
      `[windmill.worker] built ${data.meshes.length} meshes in ${(t1 - t0).toFixed(1)}ms (transferring ${transferList.length} buffers)`
    );
